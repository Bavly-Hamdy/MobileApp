...
class PDFExportService {
  ...
  private addHealthRisks(doc: jsPDF, diabetesRisk: number, heartDiseaseRisk: number, yPos: number): number {
    doc.setFontSize(16);
    doc.setFont('helvetica', 'bold');
    ...
    doc.setFontSize(12);
    doc.setFont('helvetica', 'normal');
    doc.text(`Diabetes Risk: ${diabetesRisk}%`, 20, yPos);
    yPos += 7;
    doc.text(`Heart Disease Risk: ${heartDiseaseRisk}%`, 20, yPos);
    yPos += 7;
  ...
  async exportToPDF(data: ExportData): Promise<void> {
    try {
      ...
      // Add health risks
      const dRisk = data.diabetesRisk ?? 35;
      const hRisk = data.heartDiseaseRisk ?? 25;
      yPos = this.addHealthRisks(doc, dRisk, hRisk, yPos);
