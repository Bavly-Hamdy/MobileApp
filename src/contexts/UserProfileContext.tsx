    ...
    const unsubscribeAuth = onAuthStateChanged(auth, async (user) => {
      ...
      if (user) {
        ...
        try {
          ...
          // Try to get fresh profile data immediately
          try {
            const freshProfile = await firebaseService.getUserProfile();
            if (freshProfile) {
            ...
            console.warn('UserProfileProvider: Failed initial fetch (possibly offline), falling back to subscription:', fetchError);
            // Don't set hasError to true here, let the subscription try to resolve it from cache or network when it comes back
          }
          
          // Set up real-time subscription for ongoing updates (happens regardless of initial fetch success)
          ...
          console.error('UserProfileProvider: Critical error setting up profile:', error);
          setHasError(true);
          setProfile(null);
        } finally {
          setIsLoading(false);
